...
public class Unpause {
    ...
    static void unpause(Neow3j neow3j, Hash160 bridgeContractHash, Account governor) throws Throwable {
        ...
        Transaction tx = bridge.invokeFunction("unpauseBridge")
                .signers(calledByEntry(governor))
                .sign();

        NeoSendRawTransaction rawTxResponse = tx.send();
        if (rawTxResponse.hasError()) {
            throw new Exception("Error unpausing bridge: " + rawTxResponse.getError().getMessage());
        }
