...
public class UnpauseSending {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        System.out.println("Unpause sending in message bridge...");
        printNetwork(neow3j);
    ...
    static void unpauseSending(Neow3j neow3j, Hash160 contractHash, Account governor) throws Throwable {
        SmartContract bridge = new SmartContract(contractHash, neow3j);
        if (!bridge.callFunctionReturningBool("sendingIsPaused")) {
            System.out.println("\nMessage Sending is not paused - no action needed");
            return;
        }
        System.out.println("\nMessage Sending is paused, proceeding to unpause...");

        Transaction tx = bridge.invokeFunction("unpauseSending")
                .signers(calledByEntry(governor))
                .sign();

        NeoSendRawTransaction rawTxResponse = tx.send();
        if (rawTxResponse.hasError()) {
            throw new Exception("Error unpausing Message Sending: " + rawTxResponse.getError().getMessage());
        }
