...
public class Unpause {
    ...
    static void unpause(Neow3j neow3j, Hash160 contractHash, Account governor) throws Throwable {
        ...
        Transaction tx = bridge.invokeFunction("unpause")
                .signers(calledByEntry(governor))
                .sign();

        NeoSendRawTransaction rawTxResponse = tx.send();
        if (rawTxResponse.hasError()) {
            throw new Exception("Error unpausing MessageBridge: " + rawTxResponse.getError().getMessage());
        }
