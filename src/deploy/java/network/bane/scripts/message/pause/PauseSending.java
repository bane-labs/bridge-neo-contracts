...
public class PauseSending {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        pauseSending(neow3j, bridgeHash, governorAcc);
    }

    private static void pauseSending(Neow3j neow3j, Hash160 contractHash, Account governor) throws Throwable {
        ...
        Transaction tx = bridge.invokeFunction("pauseSending")
                .signers(calledByEntry(governor))
                .sign();

        NeoSendRawTransaction rawTxResponse = tx.send();
        if (rawTxResponse.hasError()) {
            throw new Exception("Error pausing Message Sending: " + rawTxResponse.getError().getMessage());
        }
