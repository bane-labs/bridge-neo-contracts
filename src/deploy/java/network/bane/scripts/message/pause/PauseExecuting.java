...
public class PauseExecuting {
    ...
    public static void main(String[] args) throws Throwable {
        ...
        pauseExecuting(neow3j, bridgeHash, governorAcc);
    }

    private static void pauseExecuting(Neow3j neow3j, Hash160 contractHash, Account governor) throws Throwable {
        ...
        System.out.println("\nMessage Executing is not paused, proceeding to pause...");

        Transaction tx = bridge.invokeFunction("pauseExecuting")
                .signers(calledByEntry(governor))
                .sign();

        NeoSendRawTransaction rawTxResponse = tx.send();
        if (rawTxResponse.hasError()) {
            throw new Exception("Error pausing Message Executing: " + rawTxResponse.getError().getMessage());
        }
